...
//best (and since no work needs to be done in the body, can implicit return!)
const getRicksAsString = ({c137, c186}) => (`${c137}, ${c186}`);
```

*Complex Object Destructuring Examples*
```javascript
//with defaulting parameters
const westerosFamilyParser = (westerosFamilies) => {
    //If accessing westerosFamilies.northernKingdom would return undefined,
    //it is then automatically assigned to {}
    let {northernKingdom = {}} = westerosFamilies;

    return northernKingdom;
}

//with renaming
const westerosFamilyParser = (westerosFamilies) => {
    //northernKingdom is now renamed to familiesInTheNorth
    let {northernKingdom: familiesInTheNorth} = westerosFamilies;

    return familiesInTheNorth;
}

//with deep access
const westerosFamilyParser = (westerosFamilies) => {
    //this assumes the existence of northernKingdom and stark
    let {northernKingdom: {stark: {robb}}} = westerosFamilies;
...
//all combined
const westerosFamilyParser = (westerosFamilies) => {
    //Deep nested, with default (usual usecase) such that if stark
    //is undefined no access errors occur. 
    let {
        northernKingdom: {
            stark: {
                robb: kingInTheNorth
...
// bad
let mrPoopy = arr[0];
let birdPerson = arr[1];

// good
let [mrPoopy, birdPerson] = arr;
```
**[⬆ back to top](#table-of-contents)**
